...
def _validate_inputs(args: argparse.Namespace) -> None:
    ...
    if args.height % 8 != 0 or args.width % 8 != 0:
        raise ValueError(
            f"--height and --width must be multiples of 8; "
            f"got ({args.height}, {args.width})"
        )
    if (args.height // 8) % 2 != 0 or (args.width // 8) % 2 != 0:
        raise ValueError(
            f"--height/8 and --width/8 must be even for Wan DiT patch_size=(1,2,2); "
            f"got latent ({args.height // 8}, {args.width // 8})"
        )
...
def main() -> None:
    ...
    result = run_stage_a(
        ...
        resolution_hw=(int(args.height), int(args.width)),
